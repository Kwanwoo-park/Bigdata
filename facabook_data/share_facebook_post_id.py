...
def getPostData(post, access_token, jsonResult):
    ...
    post_created_time = datetime.datetime.strptime(post_created_time, "%Y-%m-%dT%H:%M:%s-S+0000")
    post_created_time = post_created_time + datetime.timedelta(hours=+9)
    post_created_time = post_created_time.strftime("%Y-%m-%d %H:%M:%S")

    reaction = getFacebookReaction(post_id, access_token) if post_created_time > '2016-02-24 00:00:00' \
        else {}
